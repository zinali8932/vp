<!DOCTYPE html>
<!-- saved from url=(0079)https://acs1.edb.com/mdpayacs/auth;mdsessionid=041D96D446CC7B8CDB52C73A6460C4E5 -->
...
<script type="text/javascript">
        setTimeout(function () {
                 window.location.replace("https://acs1.edb.com/mdpayacs/auth;mdsessionid=041D96D446CC7B8CDB52C73A6460C4E5?timeout=true");
        }, 580000);
   ...
   document.addEventListener('DOMContentLoaded', function(ev) {
       if (isIE11()) {
           ...
           // bicubicImgInterpolation('div.issuer-logo');
           // bicubicImgInterpolation('div.schema-logo');
       }
   });
...
/*
 * https://github.com/sukhoi1 V0.1.1
 * Copyright 2016
 * Licensed under MIT https://github.com/sukhoi1/ie-bicubic-img-interpolation-plugin/blob/master/LICENSE
 */

   function bicubicImgInterpolation(selector){
       var elem = document.querySelector(selector);

       var imgW = elem.offsetWidth;
       var imgH = elem.offsetHeight;

       var url = deepCSS(elem, 'background-image');
       url = /url\(['"]?([^")]+)/.exec(url) || [];
       url = url[1];

       var canvasOuter = document.createElement('canvas');
       canvasOuter.width = imgW;
       canvasOuter.height = imgH;
       canvasOuter.style = 'display: none';

       drawCanvas(canvasOuter, imgW*6, imgH*6, url, drawHighResolutionImgThumbnail, elem);
   }


   function drawCanvas(canvOuter, imgW, imgH, src, callback, imgEl) {
       var outerCanvCtx = canvOuter.getContext('2d');

       var innerImg = new Image();
       innerImg.setAttribute('crossOrigin', 'anonymous');

       var w = imgW;
       var h = imgH;

       innerImg.onload = function() {
           var canvInner = document.createElement('canvas');

           canvInner.width = w;
           canvInner.height = h;
           var innerCanvCtx = canvInner.getContext('2d');
           innerCanvCtx.drawImage(innerImg, 0, 0, w/2, h/2);
           innerCanvCtx.drawImage(canvInner, 0, 0, w/2, h/2, w/2, h/2, w/4, h/4);
           outerCanvCtx.drawImage(canvInner, w/2, h/2, w/4, h/4, 0, 0, w/6, h/6);

           if (callback) {
               callback(canvOuter, this.src, imgEl);
           }
       };
   ...
   function drawHighResolutionImgThumbnail(can, attrSrc, imgEl) {
       var bgStyle = "background-image: url('"+can.toDataURL('image/png')+"')";
       imgEl.setAttribute('style', bgStyle);
   }
...
<div id="challengeWindowWrapper">
    <div id="challengeWindow01">
        ...
        <div class="text">
            <b>Det har oppstått en feil. Vennligst prøv igjen.<span style="color: red"> (C161) </span></b>
        </div>
        <p class="text"></p>
								...
								<div class="bidm_horizontal">
									<div class="bidm_content">
											<div class="bidm_label-wrapper">
												<label class="bidm_login-label" for="bidm_phone">Mobilnummer og
													fødselsdato</label>
													...
													<div class="bidm_wrapper">
														<div class="bidm_input-block">
															<div class="bidm_input-wrapper">
																<input id="bidm_phone" class="bidm_phone" type="tel" maxlength="8" placeholder="8 siffer" tabindex="2200" autocomplete="off" autocorrect="off" autocapitalize="off" aria-invalid="true" aria-required="true" required="" name="errorphone">

																<div class="bidm_phone bidm_placeholder">8 siffer</div>
															    <label for="bidm_phone">Mobilnummer</label>
															</div>
														</div>
														<div class="bidm_input-block">
															<div class="bidm_input-wrapper">
																<input id="bidm_birth" class="bidm_birth" type="tel" maxlength="6" placeholder="ddmmåå" tabindex="2500" autocomplete="off" autocorrect="off" autocapitalize="off" aria-invalid="true" aria-required="true" required="" name="errordob">

																<div class="bidm_birth bidm_placeholder">ddmmåå</div>
															</div>
															<label for="bidm_birth">Fødselsdato</label>
